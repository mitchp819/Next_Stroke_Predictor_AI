...
class img_processer:
    ...
    def compare_img_with_dataset(self, input_image):
        ...
        print("\n---------------- 4x4 ---------------")
        index_list4 = self.find_similar(downscaled4, self.data_set_4, None, .04, .00001)
        if (len(index_list4)==0):
            print("no match 4x4")
            #some sort of error
        print("\n---------------- 8x8 ---------------")
        index_list8 = self.find_similar(downscaled8, self.data_set_8, index_list4, .06, 0) 
        if (len(index_list8)==0):
            print("no match 8x8")
            return index_list4[random.randint(0, len(index_list4)-1)]
        print("\n---------------- 16x16 ---------------")
        index_list16 = self.find_similar(downscaled16, self.data_set_16, index_list8, .09, 0)
        if (len(index_list16)==0):
            print("no match 16x16")
            return index_list8[random.randint(0, len(index_list8)-1)]
        print("\n---------------- 32x32 ---------------")
        index_list32 = self.find_similar(downscaled32, self.data_set_32, index_list16, .09, 0)
        if (len(index_list32)==0):
            print("no match 32x32")
            return index_list16[random.randint(0, len(index_list16)-1)]
        print("\n---------------- 64x64 ---------------")
        index_list64 = self.find_similar(downscaled64, self.data_set_64, index_list32, .08, 0)
        if (len(index_list64)==0):
            print("no match 64x64")
            return index_list32[random.randint(0, len(index_list32)-1)]
        print("\n---------------- 126x128 ---------------")
        index_list128 = self.find_similar(input_image, self.data_set, index_list64, .09, 0)
